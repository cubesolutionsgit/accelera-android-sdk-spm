...
dependencies {
    implementation("com.github.cubesolutionsgit:accelera-android-sdk-spm:0.6.1")
}
```
...
SDK не сохраняет контент в `onSaveInstanceState` и не передаёт его через Intent-extras — JSON-данные баннеров и сторис могут занимать сотни КБ и переполнять лимит Binder-транзакции (~1 МБ, `TransactionTooLargeException`). Вместо этого всё состояние живёт в памяти процесса:

| Что | Где хранится | Когда очищается |
|---|---|---|
| JSON загруженного баннера | LRU-кэш (до 4 МБ) по ключу запроса | Вытеснение по LRU / закрытие баннера |
| JSON сторис/попапа | Реестр payload'ов, в Intent уходит только токен | Закрытие экрана |
| Позиция просмотра сторис (entry, карточка, таймер, позиции видео) | Снапшот по токену payload | Закрытие экрана |
| Scope DivKit-переменных | Реестр scope по токену | Закрытие экрана |

**Поворот экрана / смена темы** (пересоздание Activity):

- Баннер отображается мгновенно из кэша, без повторного запроса к сети.
- Сторис продолжаются с той же карточки: таймер и видео/аудио — с того же места, DivKit-переменные (`set_variable`) сохраняются.

**Смерть процесса** (приложение выгружено в фоне):

- Кэши пусты, поэтому баннер загрузится заново, а `FullscreenActivity`/`PopupActivity` аккуратно закроются — без крашей и без устаревшего контента.

Никакой настройки со стороны приложения не требуется.
