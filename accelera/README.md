# Accelera Android SDK

Библиотека `Accelera` — это Android SDK для интеграции динамического контента (баннеров и сторис) в ваше Android-приложение с использованием Yandex DivKit.
...
Accelera.shared.delegate = object : DefaultAcceleraDelegate() {
...
- `POST /api/v1/content` - Загрузка контента (баннеры/сторис)
- `POST /api/v1/events` - Логирование событий

## 🏗️ Структура библиотеки

- `ai.accelera.library` - Основные классы (Accelera, AcceleraConfig, AcceleraDelegate)
- `ai.accelera.library.api` - Сетевой слой (AcceleraAPI, AcceleraAPIProtocol)
- `ai.accelera.library.networking` - HTTP клиент (WebClient, NetworkError)
- `ai.accelera.library.banners` - Модуль баннеров (DivKitSetup, AcceleraBanners, FullscreenActivity)
- `ai.accelera.library.compose` - Compose интеграция (AcceleraBanner, AcceleraStories, AcceleraDivView)
